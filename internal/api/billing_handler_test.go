...
func TestBillingSubscriptionShowsActivePaidPeriod(t *testing.T) {
	...
	end := time.Date(2026, time.October, 25, 12, 34, 56, 0, time.UTC)
	if err := b.db.ApplyPlanPayment(ctx, database.Payment{Reference: "ref-active", TenantID: org.ID, Plan: billing.PlanPlus, Amount: 600, Currency: "USD"}, end); err != nil {
		t.Fatal(err)
	}
	...
	if got.TenantID != org.ID || got.Plan != billing.PlanPlus || got.Status != "active" || got.CurrentPeriodEnd == nil || *got.CurrentPeriodEnd != end.Format(time.RFC3339) {
		t.Fatalf("subscription = %+v, want plus active through %s", got, end.Format(time.RFC3339))
	}
}
