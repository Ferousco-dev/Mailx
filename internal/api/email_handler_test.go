...
func TestSendUrgentPriorityAccepted(t *testing.T) {
	mux, db, tenantID := setupSendMux(t)
	rec := doJSON(t, mux, "POST", "/v1/emails", map[string]any{
		"from": "a@example.com", "to": []string{"b@example.com"}, "text": "your code is 123456", "priority": "urgent",
	})
	if rec.Code != http.StatusAccepted {
		t.Fatalf("%d %s", rec.Code, rec.Body.String())
	}
	...
	// Durably persisted, not just echoed in the response.
	msg, err := db.GetMessage(context.Background(), tenantID, got.ID)
	if err != nil || msg.Priority != "urgent" {
		t.Fatalf("%+v %v", msg, err)
	}
}

// No priority field means "normal" — unchanged pre-existing behavior.
func TestSendDefaultPriorityIsNormal(t *testing.T) {
	mux, _, _ := setupSendMux(t)
	rec := doJSON(t, mux, "POST", "/v1/emails", map[string]any{
		"from": "a@example.com", "to": []string{"b@example.com"}, "text": "hi",
	})
...
func TestSendInvalidPriorityRejected(t *testing.T) {
	mux, _, _ := setupSendMux(t)
	rec := doJSON(t, mux, "POST", "/v1/emails", map[string]any{
		"from": "a@example.com", "to": []string{"b@example.com"}, "text": "hi", "priority": "asap",
	})
	if rec.Code != 422 {
		t.Fatalf("%d %s", rec.Code, rec.Body.String())
	}
