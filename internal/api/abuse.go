...
// orgInviteAcceptIPLimitMiddleware protects the public POST
// /v1/orgs/invites/accept route, which is unauthenticated (the invitee has
// no session or API key yet - the invite token itself is the credential)
// and, on the no-account-yet path, runs a full bcrypt hash before the
// single-use token is atomically consumed. Without a limit, concurrent
// requests against one valid invitation could exhaust server CPU on
// repeated expensive hashing before any of them wins the race (Greptile
// P1, PR #23). Reuses AuthIPRate/AuthIPBurst rather than a new bucket:
// this route is the same cost class as Login (one bcrypt op per call) and
// carries no extra email-sending risk that would justify a tighter budget
// like PasswordResetIPRate's.
func orgInviteAcceptIPLimitMiddleware(a *AbuseControls) func(http.Handler) http.Handler {
	return func(next http.Handler) http.Handler {
		if a == nil || a.Limiter == nil {
			return next
		}
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			p := a.Policy
			dec, err := a.Limiter.Allow(r.Context(),
				ratelimit.Bucket{Key: "org:invite:accept:ip:" + ip, Rate: p.AuthIPRate, Burst: p.AuthIPBurst, Cost: 1},
			)
			switch {
			case err != nil:
				a.Metrics.AbuseDecision("org_invite_accept", "unavailable")
				a.log().Error("rate_limiter_unavailable", "route_class", "org_invite_accept")
				e := newError(ErrTemporarilyUnavailable, "rate_limiter_unavailable", "request limiting is temporarily unavailable; retry later")
				e.RetryAfter = unavailableRetryAfter
				writeError(w, r, e)
			case dec.Impossible:
				a.Metrics.AbuseDecision("org_invite_accept", "impossible")
				writeError(w, r, newError(ErrInternal, "internal_error", "request limit is misconfigured"))
			case !dec.Allowed:
				a.Metrics.AbuseDecision("org_invite_accept", "limited")
				e := newError(ErrRateLimited, "org_invite_accept_rate_limited", "too many invitation-acceptance attempts from this address; retry after the interval in Retry-After")
				e.RetryAfter = ratelimit.RetryAfterSeconds(dec.RetryAfter)
				writeError(w, r, e)
			default:
				a.Metrics.AbuseDecision("org_invite_accept", "allowed")
				next.ServeHTTP(w, r)
			}
		})
	}
