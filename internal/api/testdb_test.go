...
// Mirrors internal/database/testdb_test.go's isolated-schema pattern:
// package-external tests can't reuse that file's unexported helpers, so
// it is duplicated here rather than exporting a test-only helper from
// internal/database just for this one caller.
...
func newTestDB(t testing.TB) *database.DB {
	...
	b := make([]byte, 8)
	_, _ = rand.Read(b)
	schema := "mailx_api_test_" + hex.EncodeToString(b)
