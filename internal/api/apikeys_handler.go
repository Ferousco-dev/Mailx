...
func (h *orgAPIKeyHandler) handleRotate(w http.ResponseWriter, r *http.Request) {
	...
	// Grace 0: the old key stops authenticating immediately (DEC-243).
...
// updates rows WHERE revoked_at IS NULL) and changes nothing (DEC-243).
...
// orgInviteLimitMiddleware (DEC-243): an authenticated, owner-gated,
