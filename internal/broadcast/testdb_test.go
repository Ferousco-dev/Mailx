...
// Mirrors internal/database/testdb_test.go's isolated-schema pattern (also
// duplicated in internal/api): package-external tests can't reuse that
// file's unexported helpers.
...
func newTestDB(t testing.TB) *database.DB {
	...
	b := make([]byte, 8)
	_, _ = rand.Read(b)
	schema := "mailx_broadcast_test_" + hex.EncodeToString(b)
