...
- DEC-061 [v0.28]: organizational domain for alignment = ICANN PSL registrable domain through `domain.Normalize` (offline, deterministic), not the RFC 9989 tree walk; never suffix matching. Policy discovery walks only from the domain to its organizational domain (max 8 queries).
