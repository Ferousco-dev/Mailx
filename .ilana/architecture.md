...
| `internal/dmarc` | v0.28 sender-side DMARC readiness (RFC 9989): bounded parser, PSL-based alignment, bounded policy walk, readiness model; imports no dkim/spf/transport package (adapters live in `cmd/mailx/dmarcconfig.go`) |
...
Routes: `POST /v1/emails`, `GET /v1/emails[/{id}]`, `/v1/domains` (+`/{id}`, `/{id}/verify`, DELETE), `/v1/webhooks` (+`/{id}`, `/{id}/rotate-secret`, `/{id}/deliveries`, DELETE), `GET /v1/events`, `GET /v1/domains/{id}/dkim` (+POST, `/verify`), `GET /v1/domains/{id}/spf`, `POST /v1/domains/{id}/spf/verify`, `GET /v1/domains/{id}/dmarc`, `POST /v1/domains/{id}/dmarc/verify`.
...
- Alignment (`align.go`): strict = identical canonical domain; relaxed = same organizational domain from the ICANN Public Suffix List via `domain.Normalize`; never suffix matching; `co.uk`, `com`, IDN, IP literals, private-suffix names, lookalikes never align. PSL (not the RFC 9989 tree walk) decides alignment; they differ only for public-suffix operators publishing `psd=` (limitation).
- Policy discovery: `_dmarc.<domain>`, then parents up to the organizational domain, at most 8 TXT queries; first name with DMARC records governs (multiple -> `conflict`, no fall-through; malformed -> `invalid`; temporary error -> `temporary_error`, a parent policy is never substituted); inherited records use `sp` as effective policy (`source: organizational_domain`). Bounds: 5 s, 16 concurrent verifications (503 `dmarc_verification_busy`), 64 TXT records, 2048-byte record, 32 tags, 1024-byte values, 16 report URIs; no regex/recursion; fuzzed parser.
...
- Null reverse-path: DSNs are still not transmitted; if they ever are with `MAIL FROM:<>`, SPF would use `postmaster@mailx.local` which cannot align; only an aligned DKIM signature could give DMARC. Deferred to v0.29.
...
18. DMARC (v0.28): readiness only; PSL vs RFC 9989 tree-walk differences (psd); no aggregate/failure report intake or external report-destination authorization check; no policy progression tooling; relay return-path/DKIM behavior unmodelled beyond `unknown`; DSN/null-reverse-path DMARC deferred; a DMARC verify triggers an SPF verification.
