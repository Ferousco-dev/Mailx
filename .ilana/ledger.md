...
## 2026-09-22 | ad-hoc feature | GATE PASS
User-directed (conversational, outside the numbered v0.x roadmap): after diagnosing a real test-send delay (a transient DNS timeout on delivery to feranmioresajo@icloud.com scheduled a 30-minute-away retry under DefaultBackoffPolicy), user asked what happens for a 5-minute OTP scenario. Explained DefaultBackoffPolicy's first retry alone (30 min) already exceeds a typical OTP expiry, presented two options (tighten the global default vs. a separate time-critical tier), recommended the tier (avoids turning bulk-mail transient failures into a retry-storm risk), user chose it. Implemented: `priority` field on POST /v1/emails (migration 000022 `messages.priority` + `storage.StoredMessageMetadata.Priority`), `retry.UrgentBackoffPolicy`/`UrgentAttemptLimit` (~3.2 min total exhaustion vs ~7.5h default), `worker.WithUrgentCoordinator` option routing by priority read from FileStore metadata (no DB round trip on the hot path), fully backward compatible when unset. Fixed 4 pre-existing migration-rollback tests broken by adding a new messages column (they used the app's InsertMessage, which now unconditionally references priority, to fabricate "legacy" rows after intentionally rolling back past the new migration) — added a shared `legacyInsertMessage` raw-SQL test helper using only columns present since the messages table's earliest migration; also widened one hardcoded rollback-loop bound (10 -> 40) that no longer reached far enough back now that more migrations exist (same DEF-018/020 fragility class). `go test -count=1 ./...` and `-race`: 32/32 packages pass (one confirmed-flaky, unrelated pre-existing test reproduced as a pass on isolated reruns). Mutation-checked: disabling the priority-routing condition correctly fails TestCoordinatorForSelectsByPriority. Docker Compose migration (000022) + runtime restart smoke test: clean. OpenAPI updated (SendEmailRequest.priority, Email.priority). Not yet committed at time of this entry.
